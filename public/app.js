...
// ── Кастомный relay-сервер звонков (без WebRTC P2P, без Google STUN) ──────
let callRoomId = null;
...
let callBufferQueue = [];
let callBufferProcessing = false;
let callMuted = false;
...
function initSocket() {
    ...
    socket.emit('call_register', currentUser.id);
    ...
    // ── Звонки (кастомный relay — без WebRTC/Google) ────────────────────────
    socket.on('incoming_call', ({ chatId, callerId, callType, roomId }) => {
        handleIncomingCall(chatId, callerId, callType, roomId);
    });
    
    socket.on('call_answered', ({ chatId, answererId, roomId }) => {
        document.getElementById('callStatus').textContent = 'Соединение установлено';
        // Если мы звонящий и нам пришёл answer — присоединяемся к комнате
        if (roomId && callRoomId === roomId) {
            joinCallRoom(roomId);
        }
    });
    ...
    // Чанки входящего аудио
    socket.on('call_audio_chunk', ({ chunk, userId }) => {
        if (!inCall || userId === currentUser.id) return;
        const buffer = new Uint8Array(chunk).buffer;
        appendAudioChunk(buffer);
    });

    // Видеокадры (JPEG base64)
    socket.on('call_video_frame', ({ frame, userId }) => {
        if (!inCall || userId === currentUser.id) return;
        const remoteImg = document.getElementById('remoteVideoImg');
        if (remoteImg) remoteImg.src = frame;
    });

    // Участник присоединился к комнате
    socket.on('call_participant_joined', ({ userId }) => {
    ...
    socket.on('call_participant_left', ({ userId }) => {
        if (userId !== currentUser.id) {
            endCall();
        }
    });
...
function createMessageElement(msg) {
    ...
    if (msg.type === 'image') {
        ...
        const ext = msg.fileUrl ? msg.fileUrl.split('.').pop().toLowerCase() : 'webm';
        const label = ext === 'mp3' ? '🎵 Голосовое сообщение (MP3)' : '🎤 Голосовое сообщение';
        content = `
            <div class="voice-msg-label">${label}</div>
            <audio src="${msg.fileUrl}" class="message-audio" controls preload="metadata"></audio>
        `;
...
async function startCall(type) {
    ...
    try {
        ...
        // Показываем локальное видео если видеозвонок
        if (type === 'video') {
        ...
        // Создаём комнату на нашем relay-сервере
        callRoomId = 'call_' + currentChat.id + '_' + Date.now();
        socket.emit('call_create_room', {
            roomId: callRoomId,
            callerId: currentUser.id,
            callType: type
        });
        socket.emit('call_register', currentUser.id);
        socket.join && socket.join('call_' + callRoomId);
        ...
        // Присоединяемся к комнате сами
        socket.emit('call_join_room', { roomId: callRoomId, userId: currentUser.id });
        
        // Запускаем стриминг аудио
        startAudioStreaming(callLocalStream);
        
        // Если видео — запускаем стриминг кадров
        if (type === 'video') startVideoStreaming(callLocalStream);
        
        // Настраиваем приём входящего аудио
        setupRemoteAudio();
...
async function handleIncomingCall(chatId, callerId, callType, roomId) {
    const callerUser = chatId.startsWith('group_') ? 'Групповой звонок' : 'Входящий звонок';
    if (!confirm(`${callerUser}. Ответить?`)) return;
    ...
    try {
        ...
        // Присоединяемся к комнате relay-сервера
        socket.emit('call_join_room', { roomId: callRoomId, userId: currentUser.id });
        ...
        // Запускаем стриминг аудио
        startAudioStreaming(callLocalStream);
        if (callType === 'video') startVideoStreaming(callLocalStream);
        
        // Настраиваем приём входящего аудио
        setupRemoteAudio();
        
        // Открываем интерфейс звонка
        document.getElementById('callOverlay').classList.add('active');
...
function endCall() {
    ...
    if (callAudioRecorder && callAudioRecorder.state !== 'inactive') {
        try { callAudioRecorder.stop(); } catch(e) {}
    }
    callAudioRecorder = null;
    
    // Останавливаем видеостриминг
    if (callVideoInterval) {
        clearInterval(callVideoInterval);
        callVideoInterval = null;
    }
    ...
    callBufferQueue = [];
    callBufferProcessing = false;
    
    // Покидаем комнату
    if (callRoomId) {
        socket.emit('call_leave_room', { roomId: callRoomId, userId: currentUser.id });
    }
...
// ── Стриминг аудио через Socket.IO relay ────────────────────────────────
function startAudioStreaming(stream) {
    const audioStream = new MediaStream(stream.getAudioTracks());
    const mimeType = getSupportedMimeType();
    
    try {
        callAudioRecorder = new MediaRecorder(audioStream, {
            mimeType,
    ...
    callAudioRecorder.ondataavailable = async (e) => {
        if (!inCall || !e.data || e.data.size === 0) return;
        if (callMuted) return;
    ...
    callAudioRecorder.start(250); // 250ms timeslices
}
...
// ── Приём и воспроизведение входящего аудио ─────────────────────────────
function setupRemoteAudio() {
    callRemoteAudio = new Audio();
    callMediaSource = new MediaSource();
    callRemoteAudio.src = URL.createObjectURL(callMediaSource);
    callRemoteAudio.autoplay = true;
    
    callMediaSource.addEventListener('sourceopen', () => {
        try {
            const mimeType = getSupportedMimeType() || 'audio/webm;codecs=opus';
            callSourceBuffer = callMediaSource.addSourceBuffer(mimeType);
            callSourceBuffer.mode = 'sequence';
            callSourceBuffer.addEventListener('updateend', () => {
                callBufferProcessing = false;
                drainAudioQueue();
            });
        } catch(e) {
            console.warn('[CALL] MediaSource не поддерживается, fallback на Web Audio');
            setupRemoteAudioFallback();
        }
    });
...
function appendAudioChunk(buffer) {
    if (!callSourceBuffer) return;
    callBufferQueue.push(buffer);
    drainAudioQueue();
}

function drainAudioQueue() {
    if (callBufferProcessing || callBufferQueue.length === 0 || !callSourceBuffer) return;
    if (callSourceBuffer.updating) return;
    
    callBufferProcessing = true;
    const data = callBufferQueue.shift();
    try {
        callSourceBuffer.appendBuffer(data);
    } catch(e) {
        callBufferProcessing = false;
        callBufferQueue = []; // сброс при переполнении
    }
}

// Fallback: Web Audio API для браузеров без MediaSource
function setupRemoteAudioFallback() {
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    
    socket.on('call_audio_chunk', async ({ chunk, userId }) => {
        if (!inCall || userId === currentUser.id) return;
        try {
            const buffer = new Uint8Array(chunk).buffer;
...
// ── Стриминг видеокадров ─────────────────────────────────────────────────
function startVideoStreaming(stream) {
    ...
    // Создаём видеоэлемент для захвата
    ...
    callVideoInterval = setInterval(() => {
        ...
        ctx.drawImage(tmpVideo, 0, 0, 320, 240);
        const frame = canvas.toDataURL('image/jpeg', 0.5);
        socket.emit('call_video_frame', {
            roomId: callRoomId,
            frame,
            userId: currentUser.id
        });
...
// Присоединение к существующей комнате (для звонящего после ответа)
function joinCallRoom(roomId) {
    socket.emit('call_join_room', { roomId, userId: currentUser.id });
}
...
// ── Keep-alive: пингуем сервер каждые 14 минут ───────────────────────────
// Render.com усыпляет сервис после 15 мин без запросов — не даём ему заснуть
setInterval(() => {
