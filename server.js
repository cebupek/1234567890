...
const initCallServer = require('./call-server');
...
// ─── MP3 конвертация голосовых сообщений ───────────────────────────────────
function convertToMp3(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    execFile('ffmpeg', [
      ...
      if (err) {
        console.error('[MP3] Ошибка конвертации:', err.message);
        resolve(false);
      } else {
        fs.unlink(inputPath, () => {});
        resolve(true);
...
// ─── Загрузка файлов с MP3 конвертацией для голосовых ──────────────────────
app.post('/api/upload', upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Файл не загружен' });

  const fileType = req.body.type || 'files';

  if (fileType === 'voice') {
    const inputPath = req.file.path;
    const baseName = path.basename(req.file.filename, path.extname(req.file.filename));
    const mp3Filename = baseName + '.mp3';
    const mp3Path = path.join('uploads', 'voice', mp3Filename);

    try {
      const converted = await convertToMp3(inputPath, mp3Path);
      const finalFilename = converted ? mp3Filename : req.file.filename;
      const finalPath = `/uploads/voice/${finalFilename}`;
...
io.on('connection', (socket) => {
  ...
  socket.on('add_members_to_group', ({ groupId, newMembers }) => {
    ...
    if (group) {
      ...
      group.members.forEach(memberId => {
        const socketId = userSockets.get(memberId);
        if (socketId) io.to(socketId).emit('group_updated', { groupId, members: group.members });
      });
    }
  });
  ...
  socket.on('remove_member', ({ groupId, memberId, requesterId }) => {
    ...
    if (group && group.creatorId === requesterId) {
      ...
      if (removedSocketId) io.to(removedSocketId).emit('removed_from_group', { groupId });
  ...
  socket.on('clear_chat', ({ chatId, userId }) => {
    const group = groups.get(chatId);
    if (chatId.startsWith('group_') && group && group.creatorId !== userId) return;
  ...
  // ── Сигналинг звонков ─────────────────────────────────────────────────
  socket.on('start_call', ({ chatId, callerId, callType, roomId }) => {
    if (chatId.startsWith('group_')) {
      const group = groups.get(chatId);
      if (group) {
        group.members.forEach(memberId => {
          if (memberId !== callerId) {
            const socketId = userSockets.get(memberId);
            if (socketId) io.to(socketId).emit('incoming_call', { chatId, callerId, callType, roomId });
          }
        });
      }
    } else {
  ...
  socket.on('answer_call', ({ chatId, answererId, roomId }) => {
    const [user1, user2] = chatId.startsWith('group_') ? [] : chatId.split('_');
    if (chatId.startsWith('group_')) {
      const group = groups.get(chatId);
      if (group) {
        group.members.forEach(memberId => {
          const socketId = userSockets.get(memberId);
          if (socketId) io.to(socketId).emit('call_answered', { chatId, answererId, roomId });
        });
      }
    } else {
      [user1, user2].forEach(uid => {
  ...
  // Старый WebRTC сигнал (совместимость)
  socket.on('webrtc_signal', ({ to, signal, from }) => {
    const socketId = userSockets.get(to);
    if (socketId) io.to(socketId).emit('webrtc_signal', { from, signal });
  });
